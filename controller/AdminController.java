...
public class AdminController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(button != null) {
                if(button.equals("Create Story"))
                    request.getRequestDispatcher("CreateController").forward(request, response);
                else if(button.equals("Edit Story"))
                    request.getRequestDispatcher("EditStory").forward(request, response);
                else if(button.equals("Update Story"))
                    request.getRequestDispatcher("UpdateController").forward(request, response);
                else if(button.equals("Delete Story"))
